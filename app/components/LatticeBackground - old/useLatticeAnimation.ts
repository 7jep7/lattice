...
import { hexToPixel } from './hexUtils';
  ...
  export class LatticeLine {
    ...
    step(ctx: CanvasRenderingContext2D) {
      ...
      const [startX, startY] = hexToPixel(this.x, this.y, this.opts.len, this.opts.cx, this.opts.cy);
      const [endX, endY] = hexToPixel(this.x + x, this.y + y, this.opts.len, this.opts.cx, this.opts.cy);
